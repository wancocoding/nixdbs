...
setup_gentoo_mirror() {
	...
	if [ -n $mirror_area ]; then
		...
		local rsync_mirror="$(get_cfg_from_file_by_key rsync $mirror_settings_file)"
...
setup_ubuntu_mirror() {
	...
	UBUNTU_SOURCE_MIRROR_FILE="${NIXDBS_HOME}/misc/mirrors/cn/os/ubuntu/sources.${UBUNTU_CODENAME}.cn.list"
	# exe_sudo_string "echo" "'$UBUNTU_MIRROR'" ">" "/etc/apt/sources.list"
	exe_sudo_string "cat ${UBUNTU_SOURCE_MIRROR_FILE} > /etc/apt/sources.list"
	exe_sudo_string "apt update"
}
